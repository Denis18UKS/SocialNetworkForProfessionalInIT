...
const UserProfilePage = () => {
            ...
            try {
                ...
                if (!data.repositories || data.repositories.length === 0) {
                    const backupResponse = await fetch(
                        `http://localhost:5000/repositories/${data.user.github_username}`,
                        {
                            headers: {
                                Authorization: `Bearer ${token}`,
                            },
                    ...
                    if (backupResponse.ok) {
                        const backupRepos = await backupResponse.json();
                        setRepositories(backupRepos || []);
                        setFilteredRepositories(backupRepos || []);
                    }
                }
            } catch (error) {
    ...
    const handleSearch = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        const filteredRepos = source.filter((repo) =>
    ...
    const handleDownload = async (repoName: string) => {
        ...
        try {
            const response = await fetch(`http://localhost:5000/github/repos/${user?.github_username}/${repoName}/download`);
