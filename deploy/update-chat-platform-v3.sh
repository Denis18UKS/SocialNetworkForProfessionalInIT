...
BACKUP_DIR="/var/backups/socialbird/chat-platform-v3-$(date +%Y%m%d-%H%M%S)"
DEPLOY_RESTARTED=0
...
echo "[1/10] Fetching Chat Platform v3 files"
...
sudo -u "$APP_USER" git checkout "origin/$BRANCH" -- \
  ...
  deploy/fix-chat-platform-patcher-v3.mjs \
...
node deploy/fix-chat-platform-patcher-v3.mjs
...
require_text deploy/apply-chat-platform-v1.mjs "SOCIALBIRD_CHAT_PLATFORM_V3_COMPAT" "live workspace compatibility v3"
...
echo
echo "Chat Platform v3 deployed successfully."
