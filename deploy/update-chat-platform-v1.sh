...
BACKUP_DIR="/var/backups/socialbird/chat-platform-v1-$(date +%Y%m%d-%H%M%S)"
DEPLOY_RESTARTED=0
...
echo "[1/10] Fetching Chat Platform v1 files"
...
echo "Large SocialBIRD files now use 8 MiB resumable chunks, so a 125 MiB video no longer depends on one 125 MiB request."
...
echo
echo "Chat Platform v1 deployed successfully."
