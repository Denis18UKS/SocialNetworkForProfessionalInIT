...
BACKUP_DIR="/var/backups/socialbird/chat-platform-v4-$(date +%Y%m%d-%H%M%S)"
DEPLOY_RESTARTED=0
...
echo "[1/10] Fetching Chat Platform v4 files"
...
sudo -u "$APP_USER" git checkout "origin/$BRANCH" -- \
  ...
  deploy/fix-chat-platform-patcher-v4.mjs \
...
node deploy/fix-chat-platform-patcher-v4.mjs
...
require_text deploy/apply-chat-platform-v1.mjs "SOCIALBIRD_CHAT_PLATFORM_V4_COMPAT" "stable workspace/composer compatibility v4"
...
echo
echo "Chat Platform v4 deployed successfully."
